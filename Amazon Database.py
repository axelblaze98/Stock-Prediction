...
for _ in range(len(list1)):
    df = pd.read_csv(list1[_],na_values=['null'],index_col='Date',parse_dates=True,infer_datetime_format=True)
    ...
    #mycursor.execute("truncate stock_predicted;")
    
    #mycursor.execute("insert into stock_predicted(stk_id,stk_name) values (10,'ORCL')")
    
    mycursor.execute("update stock_predicted set cur_price = %s where stkid = %s;",(x, _+1))
